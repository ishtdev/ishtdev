...
class PostController extends Controller
{
    ...
    public function getPostsByHashtagData(Request $request)
    {
        try {
 //echo"<pre>"; print_r($request->all()); die;
            ...
            // $temp = post::with('postHashtag')->get();
            $hashtagMaster = HashtagMaster::where('name', $request->hashtag)->first();
            ...
            $postHashtagData = PostHashtag::select('post_id')->distinct('post_id')->where('hashtag_id',$hashtagMaster['id'])->pluck('post_id')->toArray();

            // $postIds = Post::select('id')
            //     ->where('id',$hashtagMaster['post_id'])
            //     ->pluck('id')
            //     ->toArray();
            ...
            $relatedPost = Post::whereIn('id', $postHashtagData)->get();
            ...
            // })->get();
            // echo "<pre>";
            // print_r($relatedPost->toArray());
            // die;
            $followingIds = Follows::select('followed_profile_id')->where('following_profile_id', $userProfileId)->pluck('followed_profile_id')->toArray();
            // First, get the IDs based on the hashtag filter
            // $ids = Post::select('id')
            //             ->where('caption', 'LIKE', '%' . $request->hashtag . '%')
            //             ->orderBy('id', 'DESC')
            //             ->pluck('id')
            //             ->toArray();

            // Now, use those IDs to filter the posts and apply other conditions
            // $followingPosts = [];
            // $followingPosts = Post::whereIn('profile_id', $followingIds) // Filter by profile_id
            //     ->where(function ($query) use ($userInterests) {
            //         foreach ($userInterests as $interest) {
            //             $profileInterests = explode(',', $interest); // Convert comma-separated interests
            //             foreach ($profileInterests as $related) {
            //                 // Apply the FIND_IN_SET filter for interests
            //                 $query->orWhereRaw("FIND_IN_SET(?, interest_id)", [$related]);
            //             }
            //         }
            //     })
            //     ->get(); // Apply the final filters and retrieve the results



            // $feeds = $followingPosts->concat($relatedPost)->shuffle();
            $feeds = $relatedPost->shuffle();
           ...
           // }
            ...
            //     // Check if the 'caption' key exists and is not null
            //     if (isset($post['caption'])) {
            //         // Perform case-insensitive search for the hashtag
            //         if (stripos($post['caption'], $request->hashtag) !== false) {
            //             $filteredFeeds[] = $post; // Add matching posts to the array
            //         }
            //     }
            // }
